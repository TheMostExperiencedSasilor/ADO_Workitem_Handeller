...
class TestAssignmentWorkbookService:
    ...
    def _read_execute_points(self, plan_id: int, suite_id: int) -> list[dict[str, Any]]:
        ...
        for point in points:
            ...
            configuration_name = (
                str(configuration.get("name") or configuration.get("id") or "")
                if isinstance(configuration, dict)
                else str(configuration)
            )
            point_id = point.get("id")
            rows.append({
                "testPointId": int(point_id) if point_id is not None else None,
    ...
    def build_assignment_rows(self, plan_id: int, suite_id: int, tester_query: str) -> dict[str, Any]:
        ...
        points_by_case: dict[int, list[dict[str, Any]]] = {}
        for point in candidates:
            points_by_case.setdefault(point["testCaseId"], []).append(point)

        assigned_rows: list[dict[str, Any]] = []
        for case in define_cases:
            case_points = points_by_case.get(case["testCaseId"], [])
            if not case_points:
                continue
            assigned_rows.append({
                **case,
                "tester": matched_names[0] if matched_names else query,
                "testPointIds": [point["testPointId"] for point in case_points if point["testPointId"] is not None],
                "configurations": [point["configuration"] for point in case_points if point["configuration"]],
                "round1Results": "",
                "round2Results": "",
                "singleRunResults": "",
                "manualRun": "",
        ...
        return {
            ...
            "assignedPointCount": sum(len(row["testPointIds"]) for row in assigned_rows),
